...
const getData = async () => {
  const start = calendarData.value[0][0]?.dateObj
  const end = calendarData.value[5][6]?.dateObj
  const params = {
    startTime: moment(start).format('YYYY-MM-DD'),
    endTime: moment(end).format('YYYY-MM-DD'),
  }
//   const data = await apiGetVisitData(params)
//   formatCalendar(data?.data)
}

const formatCalendar = (dataList: any) => {
  for (let i = 0; i < 6; i++) {
    for (let j = 0; j < 7; j++) {
      const target = calendarData.value[i][j]
      target.num = dataList[6 * i + j]
    }
  }
}
...
const targetDateFormat = computed(() => {
  ...
  return `${targetDate.value.getFullYear()}年${monthString}月`
})
...
<template>
  <div class="diy-calendar-wrapper">
    <div class="calendar-header">
      ...
      <!-- 年月标题 -->
      <div class="text-base center-title">{{ targetDateFormat }}</div>
      <!-- 右箭头 -->
          ...
          >
            <div class="td-content-box">
              <div class="content-border">
                <div
                  class="date-number text-base"
                  :class="{ current: isCurrentMonth(col) }"
                >
                  {{ col.dateNumber }}
                </div>
                <div v-show="col.num > 0" class="numStyle">{{ col.num }}</div>
              </div>
            </div>
          </td>
        </tr>
